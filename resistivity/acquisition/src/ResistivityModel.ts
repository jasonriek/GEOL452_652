...
class ResistivityModel {
    ...
    if (type === "Schulumberger") {
      ...
      for (this.i = 0; this.i < this.ndist; this.i++) {
        ...
        if (
          ...
          this.nterms = Math.floor(
            Math.sqrt(
              Math.pow(this.distance[this.ndist - 1], 3) / (8.0 * this.conver)
            )
          );
          if (this.nterms < this.n4 * 2) this.nterms = this.n4 * 2;
          if (this.nterms > this.maxsum) this.nterms = this.maxsum;
          q[1] = 0.0;
          for (this.m = 2; this.m <= this.nterms; this.m++) {
            ...
            if (this.m == 0) {
              ...
              this.test = this.n4;
              if (this.m - 1 < this.test) this.test = this.m - 1;
        ...
        if (
          ...
          for (this.m = 1; this.m <= this.nterms; this.m++) {
            this.temp =
              ...
              Math.pow(
                1.0 +
                  (this.m * this.m) / ((this.l * this.l) / (this.eo * this.eo)),
                -0.5
              );
            this.voltage += q[this.m] * this.temp;
          }
   ...
   */
  public p(n: number) {
    if (n === this.n2) return this.k1;
    else {
      if (n === this.n4) return this.k2;
      else return 0.0;
    }
  }
  ...
  public h(n: number) {
    if (n == 0) return 1.0;
      ...
      if (n == this.n3) return this.k1 * this.k2;
      else return 0.0;
    }
  }
   ...
   * depth_to_water
   */
  public depth_to_water(x: number, y: number) {
    let xindex, yindex, i: number;
   ...
   * depth_to_basement
   */
  public depth_to_basement(x: number, y: number) {
    let xindex, yindex, i: number;
   ...
   * getResistivity
   */
  public getResistivity() {
    res = new Array(ndata);
    for (this.i = 0; this.i < ndata; this.i++) res[this.i] = this.res[this.i];
    return res;
  }
